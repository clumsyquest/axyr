...
- `src/main.rs` — wires it together: spawns the agent thread and serves MCP over
  stdio (`get_last_crash`, `get_system_map`, `get_threads`, `get_trace`, `get_snapshot`, `get_health`, `diff_snapshot`, `list_variables`, `list_peripherals`, `list_firmware`, `read_variable`, `read_peripheral`,
